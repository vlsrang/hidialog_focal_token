...
def convert_examples_to_features(examples, max_seq_length, tokenizer, dataset='DialogRE'):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        # 본문은 tokens_a에서만 trigger 찾기 (보통 text_a는 대화 전체)
        for i in range(len(tokens_a) - len(trigger_tokens) + 1):
            if tokens_a[i:i+len(trigger_tokens)] == trigger_tokens:
                trigger_pos = i + 1  # +1은 [CLS] 위치 보정
                break

        # 못 찾은 경우 fallback
        if trigger_pos == -1:
            logger.warning(f"Trigger token '{example.trigger}' not found. Defaulting to position 1.")
            trigger_pos = 1

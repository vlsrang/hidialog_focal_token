...
ROBERTA_PRETRAINED_MODEL_ARCHIVE_LIST = [
    ...
    "deberta-large-mnli",
    "distildeberta-base",
    "deberta-base-openai-detector",
    "deberta-large-openai-detector",
    # See all RoBERTa models at https://huggingface.co/models?filter=deberta
]
...
class DebertaEmbeddings(nn.Module):
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None, speaker_ids=None, trigger_pos=None):
...
# Copied from transformers.models.bert.modeling_bert.BertSelfAttention with Bert->Deberta
class DebertaSelfAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        ...
        self.position_embedding_type = getattr(config, "position_embedding_type", "absolute")
        self.max_relative_position = getattr(config, "max_position_embeddings", 512)
        self.distance_embedding = nn.Embedding(2 * self.max_relative_position - 1, self.attention_head_size)
        self.trigger_bias_scale = nn.Parameter(torch.tensor(5.0))  # Learnable scaling factor for trigger bias
    ...
    def forward(
        ...
        if trigger_pos is not None:
            ...
            trigger_pos = trigger_pos.unsqueeze(1).unsqueeze(2)  # [B, 1, 1]
            position_ids = torch.arange(seq_len, dtype=torch.long, device=trigger_pos.device).unsqueeze(0)
            relative_pos = (position_ids - trigger_pos).abs().clamp(0, self.max_relative_position - 1)
            bias = self.distance_embedding(relative_pos)  # [B, 1, seq_len, head_dim]
            bias = bias.mean(-1, keepdim=True).permute(0, 1, 3, 2)  # [B, 1, 1, seq_len]
            bias = bias * self.trigger_bias_scale  # scale bias
...
# Copied from transformers.models.bert.modeling_bert.BertAttention with Bert->Deberta
class DebertaAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.self = DebertaSelfAttention(config)
...
# Copied from transformers.models.bert.modeling_bert.BertLayer with Bert->Deberta
class DebertaLayer(nn.Module):
    def __init__(self, config):
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = DebertaAttention(config)
...
# Copied from transformers.models.bert.modeling_bert.BertEncoder with Bert->Deberta
...
class DebertaModel(DebertaPreTrainedModel):
    ...
    # Copied from transformers.models.bert.modeling_bert.BertModel.__init__ with Bert->Deberta
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.pooler = DebertaPooler(config) if add_pooling_layer else None
    ...
    def forward(
        ...
        """
        assert trigger_pos is not None, "trigger_pos must be provided"
...
class DebertaForCausalLM(DebertaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if not config.is_decoder:
            logger.warning("If you want to use `DebertaLMHeadModel` as a standalone, add `is_decoder=True.`")
    ...
    def forward(
        ...
        Example::
            ...
            >>> from transformers import DebertaTokenizer, DebertaForCausalLM, DebertaConfig
            >>> import torch

            >>> tokenizer = DebertaTokenizer.from_pretrained('deberta-base')
            >>> config = DebertaConfig.from_pretrained("deberta-base")
            >>> config.is_decoder = True
            >>> model = DebertaForCausalLM.from_pretrained('deberta-base', config=config)
...
class DebertaForMaskedLM(DebertaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if config.is_decoder:
            logger.warning(
                "If you want to use `DebertaForMaskedLM` make sure `config.is_decoder=False` for "
...
class HiDialog_DeBERTa(DebertaPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"position_ids"]

...
from models.deberta.tokenization_deberta import DebertaTokenizer
from models.RoBERTa.HiDialog_RoBERTa import HiDialog_RoBERTa
from models.deberta.HiDialog_DeBERTa import HiDialog_DeBERTa
...
def get_logits4eval(model, dataloader, savefile, device, dataset='DialogRE'):
    ...
    for batch in tqdm(dataloader, desc="Iteration"):
        ...
        trigger_pos=batch["trigger_pos"].to(device)
...
def main():
    ...
    elif args.encoder_type == 'DeBERTa':
        config = RobertaConfig.from_json_file(args.config_file)
    else:
    ...
    elif args.encoder_type == "DeBERTa" and args.merges_file:
        tokenizer = DebertaTokenizer(vocab_file=args.vocab_file, merges_file=args.merges_file)
        special_tokens_dict = {'additional_special_tokens': ["[unused1]", "[unused2]"]} #,"[unused3]"
        num_added_toks = tokenizer.add_special_tokens(special_tokens_dict) 
    else:
    ...
    if args.encoder_type == "BERT":
        ...
        model = HiDialog_DeBERTa(
            config,
            n_class,
            gtn_num_layers=args.gtn_num_layers,
            gtn_num_channels=args.gtn_num_channels,
            dataset=args.data_name,
            intra_turn_only=args.intra_turn_only
        )
        if args.init_checkpoint is not None:
            model.load_state_dict(torch.load(args.init_checkpoint, map_location='cpu'), strict=False)
    ...
    if args.do_train:
        ...
        for ep in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_loader, desc="Iteration")):
                ...
                loss, _ = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_masks, speaker_ids=speaker_ids, graphs=graphs,  labels=label_ids, cls_indices=cls_indices, tran_ids=None, hidialog_mask=hidialog_masks, trigger_pos=batch["trigger_pos"].to(device))
                if n_gpu > 1:
                    loss = loss.mean()
            ...
            for batch in dev_loader:
                ...
                hidialog_masks = batch['hidialog_masks'].to(device)
                with torch.no_grad():
                    tmp_eval_loss, logits  = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_masks, speaker_ids=speaker_ids, graphs=graphs,  labels=label_ids, cls_indices=cls_indices, tran_ids=None, hidialog_mask=hidialog_masks, trigger_pos=batch["trigger_pos"].to(device))

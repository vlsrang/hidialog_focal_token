...
class InputExample(object):
    ...
    def __init__(self, guid, text_a, text_b=None, label=None, text_c=None, sample_id=None, answer_id=None, trigger=None):
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_id, speaker_ids, mention_ids, mentioned_h, mentioned_t, CLS_indices, hidialog_mask, transcript_id=0, answer_id=0, trigger_pos=None):
...
class bertsProcessor(DataProcessor): #bert_s
    ...
    def _create_examples(self, data, set_type):
        ...
        for (i, d) in enumerate(data):
            ...
            text_b = data[i][1]
            text_c = data[i][2]
            trigger = data[i][6] if len(data[i]) > 6 else ""
            if self.extra_info:
                sample_id = data[i][4]
                answer_id = data[i][5]
                examples.append(InputExample(guid=guid, text_a=text_a, text_b=text_b, label=data[i][3], text_c=text_c,
                sample_id=sample_id, answer_id=answer_id,trigger=trigger))
            else:
                examples.append(InputExample(guid=guid, text_a=text_a, text_b=text_b, label=data[i][3], text_c=text_c,trigger=trigger))
...
class bertsf1cProcessor(DataProcessor): #bert_s (conversational f1)
    ...
    def _create_examples(self, data, set_type):
        ...
        for (i, d) in enumerate(data):
            ...
            text_b = data[i][1]
            text_c = data[i][2]
            trigger = data[i][4] if len(data[i]) > 4 else "" 
            examples.append(InputExample(guid=guid, text_a=text_a, text_b=text_b, label=data[i][3], text_c=text_c, trigger=trigger))
...
def convert_examples_to_features(examples, max_seq_length, tokenizer, dataset='DialogRE'):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        # Trigger 단어 기준 위치 구하기
        trigger_tokens = tokenizer.tokenize(example.trigger)
        trigger_pos = -1

        # 토큰 스트링으로 비교 (subtoken 이슈 완화용)
        trigger_str = " ".join(trigger_tokens)
        tokens_a_str = " ".join(tokens_a)

        # 정확히 서브토큰까지 포함한 부분 일치 탐색
        for i in range(len(tokens_a) - len(trigger_tokens) + 1):
            window = tokens_a[i:i+len(trigger_tokens)]
            if " ".join(window) == trigger_str:
                trigger_pos = i + 1  # +1 to adjust for [CLS]
                break

        if trigger_pos == -1:
            logger.warning(f"Trigger token '{example.trigger}' not found in tokens_a. Defaulting to position 1.")
            trigger_pos = 1
        ...
        if os.path.exists(save_file):
            ...
            for f in features:
                ...
                self.data.append({
                    ...
                    'trigger_pos': np.array(f[0].trigger_pos),
                    })
                # i += 1
            # # save data
    ...
    def __iter__(self):
        ...
        for idx, minibatch in enumerate(self.batches):
            ...
            for i, example in enumerate(minibatch): # remove turn mask
                ...
                mini_input_ids = np.array([0 if v is None else v for v in mini_input_ids], dtype=np.int64)
                mini_segment_ids = np.array(mini_segment_ids, dtype=np.int64)
                mini_input_mask = np.array(mini_input_mask, dtype=np.int64)
                mini_mention_id = np.array(mini_mention_id, dtype=np.int64)
                mini_speaker_id = np.array(mini_speaker_id, dtype=np.int64)
            ...
            yield {'input_ids': get_cuda(input_ids[:cur_bsz, :batch_max_length].contiguous()),
                   ...
                   'trigger_pos': get_cuda(torch.tensor([
                        safe_int_trigger(ex['trigger_pos']) for ex in minibatch
